#!/bin/bash
#SBATCH --job-name=orig_reduced
#SBATCH --out=orig_reduced.%A_%a.log
...
orig_dataname="${dataname##*-}"

data_dir="../data/${dataname}"
prefix="${pretrained}-${max_len}"
model_dir="../${orig_dataname}_cross_entropy/${prefix}-mod"
out_dir="${prefix}-orig-reduced-out"
